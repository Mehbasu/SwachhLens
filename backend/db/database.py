...
class PostgresDB:
    ...
    def _init_db(self):
        with self.conn.cursor() as cur:
            ...
            try:
                ...
                cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS ward VARCHAR(100)")
            ...
            cur.execute("""
                INSERT INTO users (email, password_hash, role, state, district, city, ward, name, avatar_url) 
                VALUES ('officer@swachhlens.gov.in', %s, 'commissioner', 'Bihar', 'Patna', 'Patna', 'Ward 14', 'Admin Officer', NULL)
    ...
    def insert_user(self, email: str, password_hash: str, role: str, state: str = None, district: str = None, city: str = None, ward: str = None, name: str = None, avatar_url: str = None):
        if not self.conn:
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                "INSERT INTO users (email, password_hash, role, state, district, city, ward, name, avatar_url) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (email, password_hash, role, state, district, city, ward, name, avatar_url)
            )
    ...
    def get_user_by_email(self, email: str) -> dict:
        ...
        with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT email, password_hash, role, state, district, city, ward, name, avatar_url FROM users WHERE email = %s", (email,))
            return cur.fetchone()
